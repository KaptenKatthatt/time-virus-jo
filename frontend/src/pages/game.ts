...
export default function Game(socket: AppClientSocket) {
	...
	let scoreInstance: ReturnType<typeof Score>;
	let spawnTime = 0;
	...
	const setupGameDataListeners = (
		...
		socket.on("game:data", (payload: GamePayload | GamePayload[]) => {
			if (!Array.isArray(payload)) {
				...
				// ⚡ Bolt: Use .update() method to prevent expensive .innerHTML and string allocations
				scoreInstance.update(player1Data, player2Data);
	...
	const render = () => {
		...
		scoreInstance = Score(player1Data, player2Data, socket.id!);
