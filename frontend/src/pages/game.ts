...
export default function Game(socket: AppClientSocket) {
	...
	const setupGameDataListeners = (
		score: { element: HTMLDivElement; update: (p1: PlayerPayload, p2: PlayerPayload, socketId: string) => void },
		...
		socket.on("game:data", (payload: GamePayload | GamePayload[]) => {
			if (!Array.isArray(payload)) {
				...
				// ⚡ Bolt: Use targeted update to prevent layout thrashing on frequent game:data broadcasts
				score.update(player1Data, player2Data, socket.id!);
	...
	const render = () => {
		...
		const scoreObj = Score(player1Data, player2Data, socket.id!);
		...
		scoreObj.element.classList.add("game-info-card");
		...
		setupGameDataListeners(scoreObj, roundNbrEl, roundTotalEl);

		aside.appendChild(gameStatus.element);
		aside.appendChild(scoreObj.element);
